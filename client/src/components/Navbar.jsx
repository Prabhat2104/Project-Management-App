...
const Navbar = () => {
  ...
  return (
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all h-20 sticky top-0">
    ...
    <div className="relative inline-block group">
      ...
      <SquareMenu className='h-7 w-7 cursor-pointer' onMouseEnter={()=>setShowDropDown(true)}  />

      {/* Dropdown */}
      {showDropdown&&<div onMouseLeave={() => setShowDropDown(false)} className="absolute right-0 mt-2 w-40 bg-white border rounded-lg shadow-lg 
        ...
        <button onClick={()=>{setShowUpdateProfile(true)}} className="w-full text-center px-4 py-2 text-sm hover:bg-slate-100 mt-1">
          Update Profile
        </button>
        
        <button onClick={()=>{
          dispatch(logout())
          }} className="w-full text-center px-4 py-2 text-sm text-red-600 hover:bg-slate-100 mb-1">
          Logout
        </button>
      </div>}
    </div>
